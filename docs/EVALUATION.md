...
| 3.1.0 当前版本如何复核 | 使用 `requirements.lock` 和固定镜像 digest 重跑确定性报告并执行回归门禁 | [发布前检查记录](RELEASE_CHECK_3_1_0.md) |
...
3.1.0 源码候选期间，项目在固定工程文档题集上复核了“结构化上下文”候选。分块参数保持 `chunk_size=500`、`chunk_overlap=100`，Top-K 为 3，Embedding 为真实 Ollama `qwen3-embedding`（4096 维、平方 L2）；检索范围为跨文档候选池，不等同于生产单文档 `/retrieve` 范围。开发集 24 题用于选择，final 16 题仅作已曝露集合复核；final 不是未接触 holdout，不用于选择候选或移动门禁。

结果：

- 开发集：baseline 与候选均为 22/26 个证据单元完整命中；baseline 首个有效支持 MRR@3 为 0.7609，候选为 0.7029。
- final 已曝露复核：baseline 与候选均为 10/15 个证据单元完整命中；baseline MRR@3 为 0.4667，候选为 0.5000；候选将 3 个未命中转为部分覆盖，但未提升主指标完整单元 Recall@3。
- 证据单元以原文跨度为分母；允许无缺口的跨 Chunk 拼接，部分覆盖不计入完整命中，证据单元尾部空白不计入覆盖长度。

结论：候选没有改善主指标，且开发集 MRR@3 下降。项目按预设停止条件维持 Dense-only baseline，不把候选接入生产分块器、Retriever、公共契约或配置。该结果只适用于上述固定题集、真实 Embedding 身份和跨文档候选池，不构成生产单文档泛化结论。

### 独立文档题集补充复核（2026-09-23）

为补充检查结构化上下文候选在不同工程文档上的检索表现，另冻结 15 个案例：取自 `docs/MIGRATION_3_0.md` 与 `docs/RELEASE_CHECK_3_1_0.md`，含 14 个可回答案例和 1 个生产 SLA 无答案边界案例。审计确认源文档未进入 DA-03 冻结清单，`fact_family` 与 DA-03 无重合；原文、题集、评测代码及 Ollama 身份均在查看对照结果前冻结。运行使用真实 Ollama `qwen3-embedding`（4096 维，digest `64b933495768fbd3b87c20583d379728a07471e0c66733a9df87cd1901b3c44b`）、Dense Top-K=3、平方 L2、1 轮预热及 3 轮测量。

选择在运行前绑定到 R3 development 已选出的 `baseline`，未根据 holdout 结果重新选择。结果如下：

| 方案 | 可回答题完整证据单元 Recall@3 | 首个完整支持 MRR@3 | 检索未命中 | 执行失败 | 无答案空结果准确率 | P95 ms |
|---|---:|---:|---:|---:|---:|---:|
| baseline | 1.0000 | 0.8929 | 0 | 0 | 0.0000 | 145.306 |
| structured_context | 1.0000 | 0.8929 | 0 | 0 | 0.0000 | 146.301 |

每方案完成 45/45 个测量请求。边界题在两方案下均返回 3 条候选，因此无答案空结果准确率为 `0.0000`；Dense 检索未配置校准拒答阈值，该结果明确保留为拒答边界，不能写成拒答能力通过。候选未改善主指标，且 P95 略高，继续维持 Dense-only baseline。

该补充只覆盖两份文档与固定离线检索配置，不是生产单文档 `/retrieve` 或答案生成验收，不代表压力/SLA 验收，也不足以支持跨语料、跨任务或生产泛化主张。相关冻结身份、逐题结果和运行日志保存在项目私有 `agent/过程记录/DA-04-实验/holdout-20260923/`。
