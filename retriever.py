...
class Retriever:
    ...
    def __init__(self, vector_store, embedding_client):
        ...
        console.print("[green]✓ 检索器初始化完成[/green]")
    ...
    def retrieve_semantic(
        ...
        console.print(f"[green]✓ 检索到 {len(results)} 个结果[/green]")
    ...
    def retrieve_with_context(
        ...
        console.print("[yellow]上下文扩展功能待实现（需要存储时添加chunk_index）[/yellow]")
    ...
    def rerank_results(
        ...
        """
        console.print(f"\n[cyan]执行重排序...[/cyan]")
        console.print(f"  初始结果数: {len(results)}")
        ...
        console.print(f"[green]✓ 重排序完成，保留前 {top_k} 个结果[/green]")
    ...
    def display_results(results: List[RetrievalResult], title: str = "检索结果"):
        ...
        """
        if not results:
            console.print("[yellow]没有检索到相关结果[/yellow]")
            return
        ...
        for result in results:
            ...
            console.print(f"[cyan]排名 {result.rank}[/cyan] (得分: {result.score:.4f})")
            console.print(f"  来源: {result.source or '未知'} | 页码: {result.page or 'N/A'}")
...
RAG的优势在于结合了知识检索和生成能力，可以提供有据可查的准确答案。
    ...
    console.print(f"[green]✓ 已创建知识库: {kb_file}[/green]")
    console.print(f"  字符数: {len(knowledge_content):,}")
    ...
    try:
        ...
        console.print(f"[yellow]⚠ 使用真实向量失败: {str(e)}[/yellow]")
    ...
    if embedding_client is None:
        console.print("[yellow]⚠ 模拟模式，无法测试真实检索[/yellow]")
        return
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        if i == 1:  # 只在第一个查询展示上下文格式
            ...
            console.print(Panel(llm_context, border_style="green", title="供LLM使用的上下文"))
